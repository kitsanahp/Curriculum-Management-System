...
// ─── note การตีกลับต้องแสดงในเมล ─────────────────────────────────────────────

test('เมลตีกลับ (admin + คณะกรรมการ) ต้องแนบข้อเสนอแนะจากผู้ตรวจ (escape แล้ว)', async () => {
  reset();
  await emailService.sendRevisionRequired(['t@nu.ac.th'], CURRICULUM, 'แก้หมวดที่ 3 & หมวดที่ 5', null);
  assert.ok(fetchCalls[0].htmlContent.includes('ข้อเสนอแนะจากผู้ตรวจ'));
  assert.ok(fetchCalls[0].htmlContent.includes('แก้หมวดที่ 3 &amp; หมวดที่ 5'));

  reset();
  await emailService.sendCommitteeRevision(['t@nu.ac.th'], CURRICULUM, 'คณะกรรมการวิชาการ', 'ปรับตาราง <b>หมวด 4</b>', null);
  assert.ok(fetchCalls[0].htmlContent.includes('ข้อเสนอแนะจากผู้ตรวจ'));
  assert.ok(!fetchCalls[0].htmlContent.includes('<b>หมวด 4</b>'), 'note ต้องถูก escape');

  reset();
  await emailService.sendRevisionRequired(['t@nu.ac.th'], CURRICULUM, '', null);
  assert.ok(!fetchCalls[0].htmlContent.includes('ข้อเสนอแนะจากผู้ตรวจ'), 'ไม่มี note ต้องไม่มีกล่องว่าง');
});
