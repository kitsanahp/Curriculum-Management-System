...
const { uploadDocument, uploadCommittee, uploadTQF2, uploadTQF2Compare, verifyUploadSignature } = require('../middlewares/uploadMiddleware');
...
router.delete('/archived/all', authorize(ROLES.ADMIN), curriculumCtrl.forceDeleteAll);
router.post('/:id/restore', authorize(ROLES.ADMIN), curriculumCtrl.restore);
router.delete('/:id/permanent', authorize(ROLES.ADMIN), curriculumCtrl.forceDelete);

// ปีการศึกษาที่มีหลักสูตรอยู่จริง (สำหรับ dropdown ตัวกรอง) — ต้องมาก่อน /:id
router.get('/years', curriculumCtrl.getYears);
...
router.post('/:curriculum_id/documents', authorize(ROLES.ADMIN, ROLES.FACULTY, ROLES.STAFF), uploadDocument.single('file'), verifyUploadSignature, documentCtrl.upload);
router.delete('/documents/:id', authorize(ROLES.ADMIN, ROLES.FACULTY, ROLES.STAFF), documentCtrl.deleteDocument);
...
router.post('/tqf2/compare-upload',
  authorize(ROLES.ADMIN, ROLES.FACULTY, ROLES.STAFF), // executive(view-only)/registrar ไม่ควรเรียก endpoint ที่กิน CPU
...
router.post('/:curriculum_id/tqf2', authorize(ROLES.ADMIN, ROLES.FACULTY, ROLES.STAFF), uploadTQF2.single('file'), verifyUploadSignature, tqf2Ctrl.upload);
...
router.post('/committee-steps/:step_id/decision', authorize(ROLES.ADMIN),
  uploadCommittee.array('files', 10), verifyUploadSignature, committeeCtrl.uploadDecision);
