...
function diffKey(idA, idB) {
  // ห้าม sort id — diff มีทิศทาง (idA=เดิม → <del>, idB=ใหม่ → <ins>)
  // ถ้า sort จะทำให้เรียกสลับ a/b ได้ผล ins/del กลับด้านจาก cache เดิม (cache poisoning)
  return `${Number(idA)}_${Number(idB)}`;
}
...
function getHtml(docId) {
  ...
  entry.ts = Date.now(); // refresh ตอนใช้งาน → eviction เป็น LRU ไม่ใช่ FIFO (ไฟล์ immutable)
  return entry.html;
}
...
function getDiff(idA, idB) {
  ...
  entry.ts = Date.now(); // refresh ตอนใช้งาน → LRU
