<template>
  <div class="max-w-7xl mx-auto space-y-5">
    ...
    <!-- KPI bar — สรุปสถานะหลักสูตร (แนวนอนบนสุด) -->
    <div class="bg-white rounded-2xl border border-gray-200/70 shadow-sm overflow-hidden">
      ...
      <div v-if="loading" class="flex flex-wrap">
        <div v-for="i in 5" :key="i" class="px-5 py-4 border-l border-gray-100/70 first:border-l-0">
          <div class="h-12 w-28 bg-gray-100 animate-pulse rounded-lg" />
        </div>
      </div>
      <div v-else class="flex flex-wrap">
        ...
        <RouterLink to="/curricula"
          ...
          <div class="min-w-0">
            ...
            <p class="text-xs text-gray-500 mt-1 leading-tight">{{ m?.label ?? 'หลักสูตรทั้งหมด' }}</p>
          </div>
        </RouterLink>
    ...
    <!-- คู่ — ซ้าย: รออัพโหลดเอกสาร | ขวา: ต้องแก้ไขเอกสาร -->
    <div class="grid md:grid-cols-2 gap-6">

      <!-- Action Queue — รออัพโหลดเอกสาร -->
      <div class="bg-white rounded-2xl border border-gray-200/70 shadow-sm overflow-hidden flex flex-col">
        <div class="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 bg-gray-50/70">
          <div class="flex items-center gap-2">
            <span class="text-sm font-bold text-gray-900">หลักสูตรรอภาควิชาส่งเอกสาร</span>
            <span v-if="!loading && filteredPendingUploadItems.length"
              ...
              {{ filteredPendingUploadItems.length }}
            </span>
          </div>
          <RouterLink to="/curricula"
        ...
        <div v-else-if="!filteredPendingUploadItems.length"
          ...
          <p class="text-sm font-medium text-gray-500">ไม่มีหลักสูตรรอภาควิชาส่งเอกสาร</p>
        </div>

        <div v-else class="divide-y divide-gray-50 flex-1">
          <RouterLink
            v-for="c in filteredPendingUploadItems" :key="c.id" :to="`/curricula/${c.id}`"
      ...
      <!-- Revision Needed — ต้องแก้ไขเอกสาร -->
      <div class="bg-white rounded-2xl border border-gray-200/70 shadow-sm overflow-hidden flex flex-col">
        <div class="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 bg-gray-50/70">
          <div class="flex items-center gap-2">
            <span class="text-sm font-bold text-gray-900">หลักสูตรต้องแก้ไขเอกสาร</span>
...
import CommitteeProgressTracker from '@/components/dashboard/CommitteeProgressTracker.vue';
import {
  PhCaretRight, PhClock, PhCheckCircle, PhBookBookmark, PhStack,
...
const filteredPendingUploadItems = computed(() =>
